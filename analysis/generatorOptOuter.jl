...
push!(JuliaInterpreter.compiled_modules, MAT)
push!(JuliaInterpreter.compiled_modules, HDF5)

# import GeneratorSE
include("$path/../src/GeneratorSE.jl")
include("$path/objectiveOuter.jl")
...
for iRadius_con = 1:3
    for i = 4:2:8
        ...
        eff = zeros(nrow,ncol)
        xopts = zeros(nrow,ncol,18)
        base_eff = zeros(nrow,ncol)
        base_cost = zeros(nrow,ncol)
        base_xopts = zeros(nrow,ncol,18)
        ...
        #warmstart
        x0 = [4.0,1.7,0.005,0.04,0.06,0.7,0.04,5e-3,1.9,3300 / sqrt(3),0.04,0.05,0.053,1.34,1.1]
        lb = x0.*0.5
        ub = x0.*2.0
                ...
                for multisolve = 1#:nmulti+1
                    ...
                    if !isnan(torque[i_row,i_col])
                        ...
                        while (info==:Infeasible_Problem_Detected && resolve<20) #|| info!=:Solved_To_Acceptable_Level
                            ...
                            mywrap(constraints,x) = GenCostObjConOuter(;constraints,
                            r_s = x[1],
                            l_s = x[2],
                            h_m = x[3],
                            h_ys = x[4],
                            ...
                            optionsSNOPT = Options(solver=SNOPT(options=mySNOPToptions),derivatives=ForwardFD())
                            ...
                            optionsIPOPT = Options(solver=IPOPT(myIpoptoptions),derivatives=ForwardFD())

                            xopt, fopt, info = SNOW.minimize(mywrap, x0, ng,lb,ub,lg,ug,optionsIPOPT)

                            # Get the other parameters, particularly efficiency
                            costin,gen_eff,Copper,Iron,R_out,Structural_mass,Mass,mass_PM,cm,I,lcoe = GenCostObjConOuter(;r_s = xopt[1],
                            l_s = xopt[2],
                            h_m = xopt[3],
                            h_ys = xopt[4],
                            h_yr = xopt[5],
                        ...
                        while (info==:Infeasible_Problem_Detected && resolve<2)
                            ...
                            mywrap2(constraints,x) = GenCostObjConOuter(;constraints,
                            r_s = x[1],
                            l_s = x[2],
                            h_m = x[3],
                            h_ys = x[4],
        ...
        filename = "$path/PMSG_OUTER_dataGen_RefResultsOptWithBaseline_TSR$(i)_Rmax$(Rmax[iRadius_con]).mat"
